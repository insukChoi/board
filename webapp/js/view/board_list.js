...
board_list.init = function(){
    ...
    var input;
    if (!input) input = {};
    if (common.board.isNull(input["PG_NO"])) input["PAGE_NO"] = "1";
    input["PAGE_SZ"] = $("#PAGE_SZ").text();

    // 페이징 처리
    common.board.drawTablePaing("CREATE_PG_LINK", board_list.fillTable, input["PAGE_NO"], $("#CNT").val(), input["PAGE_SZ"]);
};
/*
 * 목록 조회
 */
